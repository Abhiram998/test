...
export default function Home() {
  ...
  // New Zone logic (keeping logic but respecting your design)
  ...
  const handleGenerateTicket = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: `Vehicle ${ticketData.vehicleNumber.toUpperCase()} parked successfully`,
      });
  ...
  const handleCreateZone = async () => {
    try {
      await apiPost("/api/admin/zones", {
        ...
        limits: {
          heavy: newZoneForm.heavy,
          medium: newZoneForm.medium,
          light: newZoneForm.light
        }
      });
  ...
  const handleSearch = async (e: React.FormEvent) => {
    ...
    try {
      const result = await apiGet<VehicleSearchResult>(
        `/api/search?q=${encodeURIComponent(searchQuery.toUpperCase())}`
      );
        ...
        <div className="space-y-6">
          <div className="bg-white p-6 rounded-xl shadow-sm border border-slate-100">
            ...
            <div className="h-[300px] w-full">
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={barChartData} barSize={24} margin={{ top: 20, right: 10, left: 0, bottom: 5 }}
                  onMouseMove={(state: any) => { if (state.activePayload) setHoveredZone(state.activePayload[0].payload.originalZone); }}
                  onMouseLeave={() => setHoveredZone(null)}>
                  <CartesianGrid strokeDasharray="3 3" vertical={false} stroke="#e2e8f0" />
                  <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 13, fontWeight: 500}} dy={10} interval={0} />
                  <YAxis axisLine={false} tickLine={false} tick={{fill: '#64748b', fontSize: 12}} unit="%" domain={[0, 100]} allowDataOverflow={true} />
                          ...
                          <div className="bg-white p-3 border border-slate-100 shadow-xl rounded-lg text-sm">
                            ...
                            <div className="space-y-1">
                              <div className="flex items-center justify-between gap-4 text-xs">
                                <span className="flex items-center gap-1.5 text-slate-500"><div className="w-2 h-2 rounded-full bg-[#1e293b]"></div>Heavy</span>
                                <span className="font-mono font-medium">{originalZone.stats.heavy} / {originalZone.limits?.heavy || '-'} ({data.Heavy.toFixed(1)}%)</span>
                              </div>
                              <div className="flex items-center justify-between gap-4 text-xs">
                                <span className="flex items-center gap-1.5 text-slate-500"><div className="w-2 h-2 rounded-full bg-[#f59e0b]"></div>Medium</span>
                                <span className="font-mono font-medium">{originalZone.stats.medium} / {originalZone.limits?.medium || '-'} ({data.Medium.toFixed(1)}%)</span>
                              </div>
                              <div className="flex items-center justify-between gap-4 text-xs">
                                <span className="flex items-center gap-1.5 text-slate-500"><div className="w-2 h-2 rounded-full bg-[#3b82f6]"></div>Light</span>
                                <span className="font-mono font-medium">{originalZone.stats.light} / {originalZone.limits?.light || '-'} ({data.Light.toFixed(1)}%)</span>
                              </div>
                            </div>
                          </div>
                        );
                        ...
                        <div className="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-md text-xs border border-blue-100 flex items-center gap-3">
                          <div className="flex flex-col">
                            <span className="font-bold flex items-center gap-1">
                              {searchResult.vehicle_number} 
                            </span>
                            <div className="flex items-center gap-2 text-[10px] opacity-80 mt-0.5">
                              <span className="flex items-center gap-0.5"><MapPin className="w-2.5 h-2.5"/> {searchResult.zone_name}</span>
                              <span className="flex items-center gap-0.5"><Clock className="w-2.5 h-2.5"/> {formatIST(searchResult.entry_time)}</span>
                            </div>
                          </div>
                          <Button variant="ghost" size="icon" className="h-5 w-5" onClick={() => setSearchResult(null)}>×</Button>
                        </div>
                      )}
                      <form onSubmit={handleSearch} className="flex gap-2">
                          <Input 
                            placeholder="Find Vehicle Plate..." 
                            value={searchQuery}
                            onChange={(e) => setSearchQuery(e.target.value.toUpperCase())}
                            className="bg-white w-[180px] h-9 text-sm font-mono tracking-wider"
                          />
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input value={ticketData.vehicleNumber} onChange={(e) => setTicketData({ ...ticketData, vehicleNumber: e.target.value.toUpperCase() })} className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
              ...
              <DialogFooter><Button className="w-full bg-blue-600" onClick={handleGenerateTicket}>Generate</Button></DialogFooter>
            </DialogContent>
          </Dialog>
            ...
            <DialogContent className="sm:max-w-[425px]">
              ...
              <div className="grid gap-4 py-4">
                <div className="grid grid-cols-4 items-center gap-4">
                  ...
                  <Input value={newZoneForm.name} onChange={(e) => setNewZoneForm({ ...newZoneForm, name: e.target.value })} className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label className="text-right">Heavy Cap</Label>
                  <Input type="number" value={newZoneForm.heavy} onChange={(e) => setNewZoneForm({ ...newZoneForm, heavy: parseInt(e.target.value) || 0 })} className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label className="text-right">Medium Cap</Label>
                  <Input type="number" value={newZoneForm.medium} onChange={(e) => setNewZoneForm({ ...newZoneForm, medium: parseInt(e.target.value) || 0 })} className="col-span-3" />
                </div>
                <div className="grid grid-cols-4 items-center gap-4">
                  <Label className="text-right">Light Cap</Label>
                  <Input type="number" value={newZoneForm.light} onChange={(e) => setNewZoneForm({ ...newZoneForm, light: parseInt(e.target.value) || 0 })} className="col-span-3" />
                </div>
              </div>
              <DialogFooter><Button className="w-full bg-emerald-600" onClick={handleCreateZone}>Create Zone</Button></DialogFooter>
            </DialogContent>
          </Dialog>
        </>
      )}
